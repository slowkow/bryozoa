...
SELECT COUNT(*),`rank_name` FROM `scratchpads` GROUP BY `rank_name`
  ORDER BY
...
SELECT COUNT(*),`rank_name`,`usage` FROM `scratchpads`
...
SELECT COUNT(*) FROM `scratchpads`
  WHERE `usage` = 'invalid' AND LENGTH(`unacceptability_reason`) > 1;
SELECT COUNT(*),`parent_name` FROM `scratchpads`
  GROUP BY `parent_name` ORDER BY COUNT(*) DESC LIMIT 10;
SELECT `full_name`,`taxon_author` FROM `scratchpads`
  WHERE `taxon_author` IS NULL OR `taxon_author` NOT REGEXP '[0-9]{4}'
  ORDER BY `full_name`;
SELECT COUNT(*),`unacceptability_reason` FROM `scratchpads`
